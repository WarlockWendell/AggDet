...
from .utils import reduce_sum, _transpose, iou_matrix
...
class CenterNet(nn.Module):
    ...
    def __init__(self, 
        ...
        *,
        ...
        self.agg = False if self.overlap_topk == 0 else True
    ...
    def inference(self, images, clss_per_level, reg_pred_per_level, 
        ...
        if self.only_proposal:
            proposals, topk_proposals, keep = self.predict_instances(
                grids, agn_hm_pred_per_level, reg_pred_per_level, 
                images.image_sizes, [None for _ in agn_hm_pred_per_level])
        else:
            proposals, topk_proposals, keep = self.predict_instances(
        ...
        if self.as_proposal or self.only_proposal:
            for p in range(len(proposals)):
                ...
                proposals[p].objectness_logits = proposals[p].get('scores')
                proposals[p].remove('pred_boxes')
            for p in range(len(topk_proposals)):
                topk_proposals[p].proposal_boxes = topk_proposals[p].get('pred_boxes')
                topk_proposals[p].objectness_logits = topk_proposals[p].get('scores')
                topk_proposals[p].remove('pred_boxes')
        return {
            'proposals': proposals,
            'losses': {},
    ...
    def predict_instances(
        ...
        boxlists_before_nms = boxlists
        origin_scores = boxlists_before_nms[0].scores.clone()
        _, keep1_1, keep1_2 = self.nms_and_topK(boxlists, nms=not self.not_nms)
        boxlists = []
        for i in range(len(boxlists_before_nms)):   
            ...
            boxlists_new = boxlists_before_nms.copy()
            boxlists_new[i].scores = boxlists_before_nms[i].scores.clone()
            boxlists_new[i].scores = (overlaps + scores_per_img) * 0.5
            _, keep2_1, keep2_2 = self.nms_and_topK(boxlists_new, nms=not self.not_nms)
            keep1 = torch.arange(len(boxlists_before_nms[i])).to(keep1_1.device)[keep1_1][keep1_2]
            keep2 = torch.arange(len(boxlists_before_nms[i])).to(keep1_1.device)[keep2_1][keep2_2]
            if self.overlap_topk != 0:
                keep = torch.cat([keep1, keep2], dim=0).unique() # we union the origin proposals and the proposals produced by 'overlaps'
            else:
                keep = keep1
    ...
    def nms_and_topK(self, boxlists, nms=True):
        ...
        for i in range(num_images):
            ...
            result, keep1 = ml_nms(boxlists[i], nms_thresh) if nms else boxlists[i]
            if self.debug:
