...
class CustomRCNN(GeneralizedRCNN):
    ...
    def inference(
        ...
        rpn_results = self.proposal_generator(images, features, None)
        proposals = rpn_results.get('proposals')
        topk_proposals = rpn_results.get('topk_proposals', None)
        keep = rpn_results.get('keep', None)

        if topk_proposals is not None: # for centernet2
            k = self.overlap_topk
            ious = iou_matrix(topk_proposals[0].proposal_boxes.tensor, topk_proposals[0].proposal_boxes.tensor)
            ious[torch.arange(ious.shape[0]), torch.arange(ious.shape[0])] = 0.0
            overlaps = torch.mean(torch.topk(ious, k=k, dim=-1)[0], dim=-1)[keep]
        else: # for faster rcnn
            overlaps = None 
        
        results, _ = self.roi_heads(images, features, proposals, overlaps)
        if do_postprocess:
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        rpn_results = self.proposal_generator(
            images, features, gt_instances)
        proposals = rpn_results['proposals']
        proposal_losses = rpn_results['losses']
